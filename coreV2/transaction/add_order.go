...
func (data AddLimitOrderData) Run(tx *Transaction, context state.Interface, rewardPool *big.Int, currentBlock uint64, price *big.Int) Response {
	...
	if currentPrice.Cmp(orderPrice) == -1 ||
		...
		return Response{
			...
			Log:  fmt.Sprintf("order price is %s, but must be less than %s and more than %s", orderPrice.Text('f', swap.Precision), currentPrice.Text('f', swap.Precision), maxPrice.Text('f', swap.Precision)),
