...
type UnbondDataV260 struct {
	PubKey types.Pubkey
	Coin   types.CoinID
	Value  *big.Int
}

func (data UnbondDataV260) Gas() int64 {
	return gasUnbond
}
func (data UnbondDataV260) TxType() TxType {
	return TypeUnbond
}

func (data UnbondDataV260) basicCheck(tx *Transaction, context *state.CheckState) *Response {
...
func (data UnbondDataV260) String() string {
	return fmt.Sprintf("UNBOND pubkey:%s",
		hexutil.Encode(data.PubKey[:]))
}

func (data UnbondDataV260) CommissionData(price *commission.Price) *big.Int {
	return price.Unbond
}

func (data UnbondDataV260) Run(tx *Transaction, context state.Interface, rewardPool *big.Int, currentBlock uint64, price *big.Int) Response {
	sender, _ := tx.Sender()
