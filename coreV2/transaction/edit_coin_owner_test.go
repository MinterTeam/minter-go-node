...
func TestEditOwnerTx(t *testing.T) {
	...
	response := NewExecutor(GetData).RunTx(cState, tx, big.NewInt(0), 500000, &sync.Map{}, 0, false)
	if response.Code != 0 {
...
func createDefaultValidator(cState *state.State) {
	...
	cState.Candidates.Create(types.Address{0}, types.Address{0}, types.Address{0}, types.Pubkey{0}, 0, 0, 0)
}
