...
func TestRemoveExchangeLiquidityTx_v230Ok(t *testing.T) {
	t.Parallel()
	cState := getState()
	...
	{
		data := CreateSwapPoolData{
			Coin0:   liquidityCoinID,
			Volume0: helpers.BipToPip(big.NewInt(1)),
...
func TestRemoveExchangeLiquidityTx_v230Bug(t *testing.T) {
	t.Parallel()
	cState := getState()
