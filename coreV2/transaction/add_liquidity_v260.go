...
func (data AddLiquidityDataV260) Run(tx *Transaction, context state.Interface, rewardPool *big.Int, currentBlock uint64, price *big.Int) Response {
	...
	if deliverState, ok := context.(*state.State); ok {
		...
		amount0, amount1, liquidity := deliverState.Swapper().PairMint(data.Coin0, data.Coin1, data.Volume0, data.MaximumVolume1, coinLiquidity.Volume())
		...
		tags = []abcTypes.EventAttribute{
			...
			{Key: []byte("tx.pair_ids"), Value: []byte(liquidityCoinName(data.Coin0, data.Coin1)), Index: true},
