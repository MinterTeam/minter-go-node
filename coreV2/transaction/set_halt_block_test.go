...
func TestSetHaltBlockTx(t *testing.T) {
	t.Parallel()
	cState, err := state.NewStateV3(0, db.NewMemDB(), nil, 1, 1, 500000)
	if err != nil {
	...
	response := NewExecutor(GetData).RunTx(cState, encodedTx, big.NewInt(0), 500000, &sync.Map{}, 0, false)
	if response.Code != 0 {
...
func TestSetHaltBlockTxWithWrongHeight(t *testing.T) {
	...
	cState, err := state.NewStateV3(0, db.NewMemDB(), nil, 1, 1, currentHeight)
	if err != nil {
		t.Fatalf("Cannot load state. Error %s", err)
	}
	cState.Commission.SetNewCommissions(commissionPrice.Encode())
	...
	response := NewExecutor(GetData).RunTx(cState, encodedTx, big.NewInt(0), currentHeight, &sync.Map{}, 0, false)
