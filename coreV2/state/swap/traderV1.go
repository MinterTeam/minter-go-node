...
type traderV1 struct {
}

func (t *traderV1) GetBestTradeExactIn(ctx context.Context, pairs []EditableChecker, currencyOut types.CoinID, currencyAmountIn *TokenAmount, maxHops int32) *Trade {
	return t.getBestTradeExactIn(ctx, pairs, currencyOut, currencyAmountIn, maxHops, nil, currencyAmountIn, nil)
}

func (t *traderV1) getBestTradeExactIn(ctx context.Context, pairs []EditableChecker, currencyOut types.CoinID, currencyAmountIn *TokenAmount, maxHops int32, currentPairs []EditableChecker, originalAmountIn *TokenAmount, bestTrade *Trade) *Trade {
	if maxHops <= 0 {
		return bestTrade
	}
	...
	for i, pair := range pairs {
		...
		if pair.Coin1() == tokenOut { // we have arrived at the output token, so this is the final trade of one of the paths
			...
			bestTrade = t.getBestTradeExactIn(ctx, pairsExcludingThisPair, currencyOut, NewTokenAmount(pair.Coin1(), amountOut), maxHops-1, newCurrentPairs, originalAmountIn, bestTrade)
		}
	}
...
func (t *traderV1) GetBestTradeExactOut(ctx context.Context, pairs []EditableChecker, currencyIn types.CoinID, amountOut *TokenAmount, maxHops int32) *Trade {
	return t.getBestTradeExactOut(ctx, pairs, currencyIn, amountOut, maxHops, nil, amountOut, nil)
}

func (t *traderV1) getBestTradeExactOut(ctx context.Context, pairs []EditableChecker, currencyIn types.CoinID, currencyAmountOut *TokenAmount, maxHops int32, currentPairs []EditableChecker, originalAmountOut *TokenAmount, bestTrade *Trade) *Trade {
	if maxHops <= 0 {
		return bestTrade
	}
	...
	for i, pair := range pairs {
		...
		if pair.Coin0() == tokenIn {
			...
			bestTrade = t.getBestTradeExactOut(ctx, pairsExcludingThisPair, currencyIn, NewTokenAmount(pair.Coin0(), amountIn), maxHops-1, newCurrentPairs, originalAmountOut, bestTrade)
		}
	}
