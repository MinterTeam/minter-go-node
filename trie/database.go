...
type Database struct {
	diskdb mintdb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb mintdb.Database) *Database {
	return &Database{
		diskdb: diskdb,
...
func (db *Database) insert(hash types.Hash, blob []byte) {
	...
	db.nodes[hash] = &cachedNode{
		blob:     types.CopyBytes(blob),
		children: make(map[types.Hash]int),
	}
	db.nodesSize += types.StorageSize(types.HashLength + len(blob))
}
...
func (db *Database) insertPreimage(hash types.Hash, preimage []byte) {
	...
	db.preimages[hash] = types.CopyBytes(preimage)
...
func (db *Database) dereference(child types.Hash, parent types.Hash) {
	...
	if node.parents == 0 {
		...
		db.nodesSize -= types.StorageSize(types.HashLength + len(node.blob))
	}
}
...
func (db *Database) Commit(node types.Hash, report bool) error {
	...
	for hash, preimage := range db.preimages {
		...
		if batch.ValueSize() > mintdb.IdealBatchSize {
...
func (db *Database) commit(hash types.Hash, batch mintdb.Batch) error {
	// If the node does not exist, it's a previously committed node
...
func (db *Database) Size() types.StorageSize {
	db.lock.RLock()
	defer db.lock.RUnlock()
